...
implicit none
   ...
   real(8) :: omega1, omega2, alfa2, x1, y1, cexx, ceyy, ceyy2, cexx4, radiu
...
! Constants for Flaps   
 omega1 = (alfat_transition(2) - alfat_transition(1)) / ((ifin - idebut + 1)*dt) ! Top Flap Rotational Velocity in deg/s
 omega2 = (alfab_transition(2) - alfab_transition(1)) / ((ifin - idebut + 1)*dt)  ! Bottom Flap Rotational Velocity in deg/s
 
 alfat = omega1 * (t - (idebut - 2)*dt) + alfat_transition(1)   ! Angle for top flap in deg
 alfab = omega2 * (t - (idebut - 2)*dt) + alfab_transition(1)   ! Angle for bottom flap in deg
...
do i=1,nx
    if (epsi(i,j,k).eq.0.) then
         ...
         if (x1.lt.cex+ahmlength) then
            ...
            if(y1.gt.cey) then
               sy1(i,j,k)=-sin(pi*alfat/180.)*omega1*sqrt((x1- cexx)**2 + (y1-ceyy)**2)*pi/180
               sy2(i,j,k)=cos(pi*alfat/180.)*omega1*sqrt((x1- cexx)**2 + (y1-ceyy)**2)*pi/180
            else
               sy1(i,j,k)=-sin(pi*alfab/180.)*omega2*sqrt((x1- cexx)**2 + (y1-ceyy2)**2)*pi/180
               sy2(i,j,k)=cos(pi*alfab/180.)*omega2*sqrt((x1- cexx)**2 + (y1-ceyy2)**2)*pi/180
            endif
         endif   
    endif
enddo
